<template>
    <div>
        <div class="flex items-center align-middle justify-between mb-2 py-2 " :class="headerClass">
            <div class="font-semibold my-auto">{{ $t('employee.bank_account', 2)}}</div>
            <el-button type="primary" 
            :disabled="isLoading" @click.prevent="openModal">
                <icon icon="mingcute:add-line" class="me-2"/>
                {{ `${ $t('common.add') } ${ $t('employee.bank_account') }` }}
            </el-button>
        </div>
        <el-skeleton :loading="isLoading" animated>
            ...
            <template #default v-if="data">
                <el-table class="min-w-full" v-if="data"
                    ...
                    <el-table-column prop="bank" :label="$t('employee.bank')" sortable>
                        <template #default="scope">
                            {{ scope.row.bank ?? '-'}}
                        </template>
                    </el-table-column>
                    <el-table-column prop="account_no" :label="$t('employee.account_no')" sortable>
                        <template #default="scope">
                            {{ scope.row.account_no ?? '-' }}
                        </template>
                    </el-table-column>
                    <el-table-column prop="account_name" :label="$t('employee.account_name')" sortable>
                        <template #default="scope">
        ...
        :close-on-press-escape="false">
            <el-form label-position="top" ref="formRef" :model="form" :rules="formRules" @submit.prevent="onSubmit">
                <el-row :gutter="20">
                    <el-col :md="12">
                        <el-form-item :label="$t('employee.bank')" prop="bank">
                            <el-input v-model="form.bank" clearable/>
                        </el-form-item>
                    </el-col>
                    <el-col :md="12">
                        <el-form-item :label="$t('employee.account_no')" prop="account_no">
                            <el-input v-model="form.account_no" clearable/>
                        </el-form-item>
                    </el-col>
                    <el-col :md="12">
                        <el-form-item :label="$t('employee.account_name')" prop="account_name">
...
const {
    ...
    queryKey: [`showEmployeeBankAccount-${props.employee}`, params.value], // Query key unik
    queryFn: fetchData,
...
const formRules = ref({
    bank: [
        { required: true, message: t('validation.required', { attribute: t('employee.bank') }), trigger: 'blur' },
    ],
    account_no: [
        { required: true, message: t('validation.required', { attribute: t('employee.account_no') }), trigger: 'blur' },
    ],
    account_name: [
        { required: true, message: t('validation.required', { attribute: t('employee.account_name') }), trigger: 'blur' },
    ]
});
const formLoading = ref(false);
const openModal = () => {
    formTitle.value = `${t('common.add')} ${t('employee.bank_account')}`;
    formShow.value = true;
    form.value.id = null;
    form.value.bank = null;
    form.value.account_name = null;
    form.value.account_no = null;
    form.value.file = null;
};

const onEdit = (data) => {
    formTitle.value = `${t('common.edit')} ${t('employee.bank_account')}`;
    formShow.value = true;
    ...
    formRef.value.validate(async (valid) => {
        if (valid) {
            ...
            try {
                ...
                const url = form.value.id ? `/bank-account/${form.value.id}/update` : '/bank-account/store';
                const method = 'post';
                await axios({
                    method,
                    url,
...
const onDelete = (id) => {
    ElMessageBox.confirm(t("message.delete_confirm"), t('message.delete_confirm_title'), {
        ...
        axios.delete(`/bank-account/${id}/delete`).then(() => {
            refetch();
            ElMessage({
                type: 'success',
