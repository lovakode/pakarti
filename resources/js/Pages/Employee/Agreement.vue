<template>
    <div>
        <div class="flex items-center align-middle justify-between mb-2 py-2 " :class="headerClass">
            <div class="font-semibold my-auto">{{ $t('employee.work_agreement', 2)}}</div>
            <el-button type="primary" 
            :disabled="isLoading" @click.prevent="openModal">
                <icon icon="mingcute:add-line" class="me-2"/>
                {{ `${ $t('common.add') } ${ $t('employee.work_agreement') }` }}
            </el-button>
        </div>
        <el-skeleton :loading="isLoading" animated>
            ...
            <template #default v-if="data">
                <el-table class="min-w-full"
                    ...
                    <el-table-column prop="ref" :label="$t('common.type')" sortable>
        ...
        :close-on-press-escape="false">
            <el-form label-position="top" ref="formRef" :model="form" :rules="formRules" @submit.prevent="onSubmit">
                <el-row :gutter="20">
                    <el-col :md="12">
                        <el-form-item :label="$t('employee.document_no')" prop="ref">
                            <el-input v-model="form.ref" clearable/>
                        </el-form-item>
                    </el-col>
                    <el-col :md="12">
...
const {
    ...
    queryKey: [`showEmployeeAgreement-${props.employee}`, params.value], // Query key unik
    queryFn: fetchData,
...
const formRules = ref({
    ref: [
        { required: true, message: t('validation.required', { attribute: t('employee.document_no') }), trigger: 'blur' },
    ],
    from: [
...
const openModal = () => {
    formTitle.value = `${t('common.add')} ${t('employee.work_agreement')}`;
    formShow.value = true;
    form.value.id = null;
    form.value.ref = null;
...
const onEdit = (data) => {
    formTitle.value = `${t('common.edit')} ${t('employee.work_agreement')}`;
    formShow.value = true;
    form.value.id = data.id;
    form.value.ref = data.ref;
    form.value.from = data.from;
    form.value.to = data.to;
    ...
    formRef.value.validate(async (valid) => {
        if (valid) {
            ...
            try {
                ...
                const url = form.value.id ? `/agreement/${form.value.id}/update` : '/agreement/store';
                const method = 'post';
                await axios({
                    method,
                    url,
...
const onDelete = (id) => {
    ElMessageBox.confirm(t("message.delete_confirm"), t('message.delete_confirm_title'), {
        ...
        axios.delete(`/agreement/${id}/delete`).then(() => {
            refetch();
            ElMessage({
                type: 'success',
