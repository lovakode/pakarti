<template>
    <div>
        <div class="flex items-center align-middle justify-between mb-2 py-2 " :class="headerClass">
            <div class="font-semibold my-auto">{{ $t('employee.education', 2)}}</div>
            <el-button type="primary" 
            :disabled="isLoading" @click.prevent="openModal">
                <icon icon="mingcute:add-line" class="me-2"/>
                {{ `${ $t('common.add') } ${ $t('employee.education') }` }}
            </el-button>
        </div>
        <el-skeleton :loading="isLoading" animated>
            ...
            <template #default v-if="data">
                <el-table class="min-w-full"
                    ...
                    <el-table-column :label="$t('employee.degree')" sortable>
                        <template #default="scope">
                            {{ scope.row.degree ? $t(`employee.${scope.row.degree}`) : '-'}}
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('employee.institution')" sortable>
                        <template #default="scope">
                            {{ scope.row.institution ?? '-' }}<br/>
                            {{ scope.row.field_of_study ?? '-' }}
                        </template>
                    </el-table-column>
                    <el-table-column prop="grade" :label="$t('employee.grade')" sortable/>
                    <el-table-column :label="$t('employee.graduation_date')" sortable>
                        <template #default="scope">
                            {{ scope.row.graduation_date ? formatDate(scope.row.graduation_date) : '-' }}
                        </template>
                    </el-table-column>
...
const {
    ...
    queryKey: [`showEmployeeEducation-${props.employee}`, params.value], // Query key unik
    queryFn: fetchData,
...
const openModal = () => {
    formTitle.value = `${t('common.add')} ${t('employee.education')}`;
    formShow.value = true;
    form.value.id = null;
...
const onEdit = (data) => {
    formTitle.value = `${t('common.edit')} ${t('employee.education')}`;
    formShow.value = true;
    ...
    formRef.value.validate(async (valid) => {
        if (valid) {
            ...
            try {
                ...
                const url = form.value.id ? `/education/${form.value.id}/update` : '/education/store';
                const method = 'post';
                await axios({
                    method,
                    url,
...
const onDelete = (id) => {
    ElMessageBox.confirm(t("message.delete_confirm"), t('message.delete_confirm_title'), {
        ...
        axios.delete(`/education/${id}/delete`).then(() => {
            refetch();
            ElMessage({
                type: 'success',
