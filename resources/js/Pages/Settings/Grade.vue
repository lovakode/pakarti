...
const {
    ...
    queryKey: ['warehouseList', params.value], // Query key unik
    queryFn: fetchData,
...
const openModal = () => {
    formTitle.value = `${ t('common.create') } ${ t('base.grade') }`;
    formShow.value = true;
    form.value.id = null;
...
const onEdit = (data) => {
    formTitle.value = `${ t('common.edit') } ${ t('base.grade') }`;
    formShow.value = true;
    ...
    formRef.value.validate(async (valid) => {
        if (valid) {
            ...
            try {
                ...
                const url = form.value.id ? `/settings/grade/${form.value.id}/update` : '/settings/grade/store';
...
const onDelete = (id) => {
    ElMessageBox.confirm(t("message.delete_confirm"), t('message.delete_confirm_title'), {
        ...
        axios.delete(`/settings/grade/${id}/delete`).then(() => {
            refetch();
            ElMessage({
                type: 'success',
