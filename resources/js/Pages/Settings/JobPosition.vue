...
onMounted(() => {
    setTitle(t('base.job_position', 2));
});
const params = ref({
    page: 1,
    limit: 25,
...
const {
    ...
    queryKey: ['job_positionList', params.value], // Query key unik
    queryFn: fetchData,
...
const openModal = () => {
    formTitle.value = `${ t('common.create') } ${ t('base.job_position') }`;
    formShow.value = true;
    form.value.id = null;
...
const onEdit = (data) => {
    formTitle.value = `${ t('common.edit') } ${ t('base.job_position') }`;
    formShow.value = true;
    ...
    formRef.value.validate(async (valid) => {
        if (valid) {
            ...
            try {
                ...
                const url = form.value.id ? `/settings/job_position/${form.value.id}/update` : '/settings/job_position/store';
...
const onDelete = (id) => {
    ElMessageBox.confirm(t("message.delete_confirm"), t('message.delete_confirm_title'), {
        ...
        axios.delete(`/settings/job_position/${id}/delete`).then(() => {
            refetch();
            ElMessage({
                type: 'success',
