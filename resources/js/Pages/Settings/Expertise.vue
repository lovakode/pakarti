...
const fetchData = async ({
    ...
    const response = await axios.get("/settings/expertise", {
        params: queryParams,
    });
...
const openModal = () => {
    formTitle.value = `${ t('common.create') } ${ t('base.expertise') }`;
    formShow.value = true;
    form.value.id = null;
...
const onEdit = (data) => {
    formTitle.value = `${ t('common.edit') } ${ t('base.expertise') }`;
    formShow.value = true;
    ...
    formRef.value.validate(async (valid) => {
        if (valid) {
            ...
            try {
                ...
                const url = form.value.id ? `/settings/expertise/${form.value.id}/update` : '/settings/expertise/store';
...
const onDelete = (id) => {
    ElMessageBox.confirm(t("message.delete_confirm"), t('message.delete_confirm_title'), {
        ...
        axios.delete(`/settings/expertise/${id}/delete`).then(() => {
            refetch();
            ElMessage({
                type: 'success',
