    ...
    <div class="content">
            ...
            <el-skeleton :loading="isLoading" animated>
                ...
                <template #default>
                    <el-table class="min-w-full" 
                        ...
                        <el-table-column prop="employee_count" :label="$t('base.employee')" sortable>
                            <template #default="scope">
        ...
        :close-on-press-escape="false">
            <el-form label-position="top" ref="formRef" :model="form" :rules="formRules" @submit.prevent="onSubmit">
                ...
                <el-form-item :label="$t('base.parent_department')" prop="parent_id">
                    <select-department v-model="form.parent_id"/>
                </el-form-item>
                <div class="text-end">
...
const {
    ...
    queryKey: ['departmentList', params.value], // Query key unik
    queryFn: fetchData,
...
const openModal = () => {
    formTitle.value = `${ t('common.create') } ${ t('base.department') }`;
    formShow.value = true;
    form.value.id = null;
...
const onEdit = (data) => {
    formTitle.value = `${ t('common.edit') } ${ t('base.department') }`;
    formShow.value = true;
    ...
    formRef.value.validate(async (valid) => {
        if (valid) {
            ...
            try {
                ...
                const url = form.value.id ? `/settings/department/${form.value.id}/update` : '/settings/department/store';
...
const onDelete = (id) => {
    ElMessageBox.confirm(t("message.delete_confirm"), t('message.delete_confirm_title'), {
        ...
        axios.delete(`/settings/department/${id}/delete`).then(() => {
            refetch();
            ElMessage({
                type: 'success',
